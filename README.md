...
Each config entry bundles a structure regex, whether it protects, its breach mode, and its own place/break exceptions, so you can define one set of rules for some structures and a different set for others. By default a set of challenging structures is protected, with a shared base granting the common light-source exception; you can add or remove structures as you see fit.
